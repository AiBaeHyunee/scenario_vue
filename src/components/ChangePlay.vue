                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="#!" method="post" class="row">
                            ...
                            <!--                            </div>-->
                            <div class="form-group col-md-12">
                                <button type="button" class="btn btn-primary" @click="changePlayShow">确 定</button>
...
<script>
    ...
    export default {
        ...
        mounted() {
            this.changeTitle('剧本-剧本修改')
            this.loadPlay()
        },
        methods:{
            //加载剧本信息
            loadPlay:function () {
                let playNumber = localStorage.getItem("play_number")
                console.log("根据剧本Number查询剧本")
                var that = this
                axios.get(that.GLOBAL.API_ROOT + "/show_play", {params:{
                        "playNumber":playNumber,
                    }}).then(
                    function (response) {
                        that.play_number = response.data.play.play_number
                        that.play_name = response.data.play.play_name
                        that.play_detail = response.data.play.play_detail
                        that.play_photo = response.data.play.play_photo

                        that.play_detailphoto = response.data.play.play_detail_photo

                        that.play_price = response.data.play.play_price
                        that.play_type = response.data.play.play_type
                        that.play_people = response.data.play.play_people
            ...
            //更改剧本信息
            changePlayShow:function(){
                console.log("更改剧本信息")
                let that = this
                console.log(that.play_name)
                axios.post(that.GLOBAL.API_ROOT + "/change_play", qs.stringify({
